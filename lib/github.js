  ...
  var Github = function(options) {
    ...
    Github.Repository = function(options) {
      ...
      this.write = function(branch, path, content, message, cb) {
        /*
        updateTree(branch, function(err, latestCommit) {
          console.log("update Tree", branch, err, latestCommit)
          if (err) return cb(err);
          that.postBlob(content, function(err, blob) {
            console.log("post blog", content, err, blob)
            if (err) return cb(err);
            that.updateTree(latestCommit, path, blob, function(err, tree) {
              console.log("update tree", latestCommit, path, blob, err, tree)
              if (err) return cb(err);
              that.commit(latestCommit, tree, message, function(err, commit) {
                console.log("commit", latestCommit, tree, message, err, commit)
                if (err) return cb(err);
      ...
      this.isCollaborator = function(collaborator, cb) {
        var url = repoPath + "/collaborators/" + collaborator;
        _request("GET", url, null, cb);
      }
